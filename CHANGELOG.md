
# Change Log
All notable changes to this project will be documented in this file.
 
The format is based on [Keep a Changelog](http://keepachangelog.com/)
and this project adheres to [Semantic Versioning](http://semver.org/).

## [0.1.1] 2026-04-15
    * Fix
      * fix some Error in Error management (on traces)
      * fix bugs get_value() vs get_encoded() for saving.
    * Internal
      * Add transaction in all routes with rollback
      * pushing kwargs to actions (for transactions)
    * Feature
      * Add migration method
      * Add backoffice.check_syntax()
    * Doc
      * Documentation for migration

## [0.1.0] 2026-04-08
    * Fix
      * fix issue #3
    * Feature
      * Changing Error management
      * minimize Human errors
        * Moving to Kparse for **kwargs
        * add check_json to requests
        * Adding validation parameters
    * Doc
      * Documentation refactoring


## [0.0.4] 2026-03-17

    * Example : wget connector to DB example
    * Fix: avoid current_user in some debug message (if not set)
    * Fix: RefsList bug fix for external DB.
    * Feature: Log_level wrapper to avoid importing logging
    * Feature : rexport from stricto to avoid stricto import
    * Fix : fix os.mkdir() -> os.makedirs() in Yml BD connector
    * Internal : adding loop_path.py
    * Feature : FillStrategy.NO_FILL
    * Feature : selection with POST route


## [0.0.3] 2026-03-20

 * selections feature
  
## [0.0.2]

 * sphinx documentation
 * Adding actions routes
 * increase README.md example and documentation
 * Improving selectors with Ref and RefLists
 * Habilitation in Blueprint
 * Using stricto 0.0.8
 * current_user available
 * meta routes with documentation
 * checking routes
  
## [0.0.1]

First version

### Added

### Changed

### Fixed
