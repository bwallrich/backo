...
FORMATS = {
    logging.DEBUG: logging.Formatter('%(levelname)s-%(name)s-%(pathname)s.%(lineno)d : %(message)s'),
...
FORMATS_COLOR = {
    logging.DEBUG: logging.Formatter(OKGREEN + '%(levelname)s-%(name)s-%(pathname)s.%(lineno)d : %(message)s' + RESET_COLOR),
    logging.INFO: logging.Formatter(GREY + '%(levelname)s : %(message)s' + RESET_COLOR),
    logging.WARNING: logging.Formatter(YELLOW + '%(levelname)s : %(message)s' + RESET_COLOR),
    logging.ERROR: logging.Formatter(RED + '%(levelname)s : %(message)s' + RESET_COLOR),
    logging.CRITICAL: logging.Formatter(BOLD_RED + '%(levelname)s : %(message)s' + RESET_COLOR)
}
...
class Logger:
    ...
    def set_memoryhandler( self, target, **kwargs):
        ...
        # Adding a methode to get the last message
        # use for test
        h.get_last_message = types.MethodType( lambda self : self.buffer[-1] if len(self.buffer) else None, h)
        return h
